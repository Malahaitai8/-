    ...
    <el-table :data="volunteers" style="width: 1500px; margin-top: 20px">
      ...
      <el-table-column label="操作" width="300">
        <template v-slot="scope">
          ...
          <el-button type="primary" size="mini" style="margin-left: 10px;" @click="approve(scope.row)">允许加入</el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    viewDetails(row) {
      ...
      this.$router.push('/detailed-info-for-add-activity');
    },
    approve(row) {
      console.log('允许加入：', row);
      this.$message.success('该成员已加入志愿活动！');
    }
  }
};
</script>
