    ...
    <el-table :data="filteredVolunteers" style="width: 1500px; margin-top: 20px">
      ...
      <el-table-column label="操作" width="300" align="center">
        <template #default="scope">
          <el-button type="primary" size="small" @click="deleteMember(scope.row)">剔除该成员</el-button>
...
// 表单数据和验证规则
    ...
    const fetchVolunteers = async () => {
      try {
        ...
        const res = await request.get("/volunteerOrganizationJoin/active", {
          params: { orgId }
        });
        if (res.code === '200' && res.data) {
          volunteers.value = res.data;
        } else {
          ElMessage.error(res.msg || "获取正式成员数据失败");
        }
      } catch (error) {
        ElMessage.error("获取正式成员数据时发生网络错误");
      }
    };
    ...
    const deleteMember = async (row) => {
      try {
        const orgId = orgIdStore.orgId;
        const res = await request.post("/volunteerOrganizationJoin/deleteMemberRequest", {
          volunteerId: row.volunteerId,
          orgId: orgId
        });
        // ✅ 正确判断成功条件
        if (res.code === '200') {
          ElMessage.success("成功剔除该成员");
          fetchVolunteers(); // 刷新申请列表
        } else {
          ElMessage.error(res.msg || '剔除失败');
        }
      } catch (error) {
        ElMessage.error("剔除时发生网络错误");
      }
    };
